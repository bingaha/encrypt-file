...
轻量级跨平台（Windows / Linux）文件与文件夹加密工具。编译为单个可执行文件，无需安装。

## 功能特性

- **目录模式** — 将程序放入任意文件夹，双击运行即可一键加密/解密目录下所有文件和子目录，文件名和目录名同步加密
- **单文件模式** — 加密单个文件，元数据自包含，加密文件可独立分享，接收方只需相同程序和密码即可解密还原
- **拖拽支持** — 将文件拖入窗口即可自动切换到单文件模式并选中该文件
- **零安装** — 编译为单个可执行文件（约 4MB），无运行时依赖，拷贝即用
...
- Windows：`vault-<版本号>-windows-x64.zip`（内含 `vault.exe`）
- Linux：`vault-<版本号>-linux-x64.tar.gz`（内含 `vault`）
...
产出：`target/release/vault`

需要 Rust 1.70+。中文字体从系统字体目录加载（Windows 为 `C:/Windows/Fonts`，Linux 为 `/usr/share/fonts` 等），未内嵌字体文件；若系统无中文字体，界面中文可能显示为方框。

## 使用方式

### 目录加密

1. 将程序（Windows 为 `vault.exe`，Linux 为 `vault`）复制到目标文件夹
2. 双击运行
3. 程序自动检测未加密状态，显示加密界面
4. 设置密码
5. 点击 **开始加密**
6. 加密完成后可带走程序 — 目录中只剩乱码文件名和不可读的文件内容
...
加密后名称长度会翻倍（十六进制编码），程序会校验其不超过 255 字符（Linux/Windows 共同的单文件名上限），超长文件名会在加密前被拒绝。
